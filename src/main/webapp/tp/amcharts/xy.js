AmCharts.AmXYChart = AmCharts.Class({inherits: AmCharts.AmRectangularChart, construct: function (a) {
  this.type = "xy";
  ...
  this.parseData();
  var a = this.chartData, c = a.length - 1, b = this.graphs, d = this.dataProvider, e =
      -Infinity, f = Infinity, k, g;
  for (k = 0; k < b.length; k++)if (g = b[k], g.data = a, g.zoom(0, c), g = g.valueField) {
    var m;
  ...
  this.chartData = [];
  var a = this.dataProvider, c = this.valueAxes,
      b = this.graphs, d;
  if (a)for (d = 0; d < a.length; d++) {
    var e = {axes: {}, x: {}, y: {}}, f = a[d], k;
  ...
  d || (d = this.nf);
  a = AmCharts.formatValue(a, c.values, ["value", "x", "y"], d);
  ...
  for (c = 0; c < a.length; c++) {
    var b = a[c], b = AmCharts.processObject(b, AmCharts.TrendLine, this.theme);
    a[c] = b;
    b.chart = this;
    var d = b.valueAxis;
  ...
  var a = this.scrollbarH;
  a && a.relativeZoom(this.widthMultiplier,
  ...
  var c = this.widthMultiplier * this.plotAreaWidth / a.selectionWidth, b = this.heightMultiplier * this.plotAreaHeight / a.selectionHeight, c = this.fitMultiplier(c), b = this.fitMultiplier(b);
