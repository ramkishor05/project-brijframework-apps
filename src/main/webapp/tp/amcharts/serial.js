AmCharts.AmSerialChart = AmCharts.Class({inherits: AmCharts.AmRectangularChart, construct: function (a) {
  ...
  if (AmCharts.ifArray(f) && (a != this.startTime ||
      b != this.endTime)) {
    var l = e.minDuration(), d = this.firstTime, n = this.lastTime;
    a || (a = d, isNaN(c) || (a = n - c));
    b || (b = n);
    a > n && (a = n);
    b < d && (b = d);
  ...
  var a = 0, b = this.valueAxes.length, c = this.graphs.length, d, e, f = !1, l, n;
  for (n = 0; n < b; n++) {
    e = this.valueAxes[n];
    var k = e.stackType;
    if ("100%" == k || "regular" == k)for (f = !1, l = 0; l < c; l++)d = this.graphs[l], d.tcc = 1, d.valueAxis == e && "column" == d.type && (!f && d.stackable &&
        (a++, f = !0), (!d.stackable && d.clustered || d.newStack) && a++, d.columnIndex = a - 1, d.clustered || (d.columnIndex = 0));
    if ("none" == k || "3d" == k)for (l = 0; l < c; l++)d = this.graphs[l], d.valueAxis == e && "column" == d.type && d.clustered && (d.tcc = 1, d.newStack && (a = 0), d.hidden || (d.columnIndex = a, a++));
    if ("3d" == k) {
      e = 1;
      for (n = 0; n < c; n++)d = this.graphs[n], d.newStack && e++, d.depthCount = e, d.tcc = a;
      a = e
    }
  }
  return a
  ...
  var l = e + Math.round((f - e) / 2), n = a[l][b];
  if (1 >= f - e) {
    if (d)return e;
    d = a[f][b];
  ...
  if (b) {
    ...
    for (var n = 0; n < c.length; n++) {
        ...
        if (z && (z = z.axes[e.id].graphs[b.id])) {
          if (z.values) {
            ...
            if (!isNaN(A)) {
              ...
              var D = AmCharts.getDecimals(A), g = g + A, g = AmCharts.roundTo(g, Math.max(p, D));
              h++;
              p = g / h
            }
          }
      ...
      v = {open: x, close: t, high: w, low: y, average: C, sum: v, count: q};
      a = AmCharts.formatValue(a, {open: m, close: u, high: s, low: r, average: p, sum: g, count: h}, d, l, k + "\\.", this.usePrefixes, this.prefixesOfSmallNumbers, this.prefixesOfBigNumbers);
...
AmCharts.Cuboid = AmCharts.Class({construct: function (a, b, c, d, e, f, l, n, k, g, h, m, u, r, s) {
  this.set = a.set();
  this.container = a;
  this.h = Math.round(c);
  this.w = Math.round(b);
  ...
  var b = this.container, c = this.w, d = this.h, e = this.dx, f =
      this.dy, l = this.colors, n = this.alpha, k = this.bwidth, g = this.bcolor, h = this.balpha, m = this.gradientRotation, u = this.cornerRadius, r = this.dashLength, s = this.pattern, p = l, v = l;
  ...
  1 > Math.abs(d) && (d = 0);
  1 > Math.abs(c) && (c = 0);
  b = 0 === d ? AmCharts.line(b, [0, c], [0, 0], g, h, k, r) : 0 === c ? AmCharts.line(b,
      [0, 0], [0, d], g, h, k, r) : 0 < u ? AmCharts.rect(b, c, d, l, n, k, g, h, u, m, r) : AmCharts.polygon(b, [0, 0, c, c, 0], [0, d, d, 0, 0], l, n, k, g, h, m, !1, r);
...
AmCharts.CategoryAxis = AmCharts.Class({inherits: AmCharts.AxisBase, construct: function (a) {
  ...
  if (AmCharts.ifArray(a)) {
    ...
    b = this.end - d + 1;
    var l = this.gridCountR, n = this.showFirstLabel, k = this.showLastLabel, g, h = "", m = AmCharts.extractPeriod(this.minPeriod);
    ...
    var x = this.firstDayOfWeek, t = this.boldPeriodBeginning, a = AmCharts.resetDateToMin(new Date(a[a.length -
        1].time + 1.05 * g), this.minPeriod, 1, x).getTime(), y;
    this.endTime > a && (this.endTime = a);
    q = this.minorGridEnabled;
    var w, a = this.gridAlpha, C;
    if (this.parseDates && !this.equalSpacing) {
      ...
      var l = Math.ceil(this.timeDifference / r) + 5, B = h = AmCharts.resetDateToMin(new Date(this.startTime - r), e,
          u, x).getTime();
      ...
      if (0 < this.gridCountR)for (b = d; b <= l; b++) {
        ...
        this.pushAxisItem(p);
        p = h = m;
        if (!isNaN(w))for (g = 1; g < u; g += w)this.gridAlpha = this.minorGridAlpha, q = m + C * g, q = AmCharts.resetDateToMin(new Date(q), e, w, x).getTime(), q = new this.axisItemRenderer(this, (q - this.startTime) * this.stepWidth), this.pushAxisItem(q);
        this.gridAlpha = a
      }
    } else if (!this.parseDates) {
      if (this.cellWidth = this.getStepWidth(b), b < l && (l = b), f += this.start, this.stepWidth = this.getStepWidth(b), 0 < l)for (t = Math.floor(b / l), w = this.chooseMinorFrequency(t), g =
        ...
        if (q && !isNaN(w))if (b / w == Math.round(b / w) || p)b / t == Math.round(b / t) || p || (this.gridAlpha = this.minorGridAlpha, h = void 0); else continue; else if (b / t != Math.round(b / t) && !p)continue;
      ...
      r = g;
      if (this.data[g].time != AmCharts.resetDateToMin(new Date(this.data[g].time), e, u, x).getTime())for (x = 0, y = h, b = g; b < v; b++)m = this.data[b].time, this.checkPeriodChange(e, u, m, y) && (x++, 2 <= x && (r = b, b = v), y = m);
      q && 1 < u && (w = this.chooseMinorFrequency(u), AmCharts.getPeriodDuration(e, w));
      if (0 < this.gridCountR)for (b = g; b < v; b++)if (m = this.data[b].time, this.checkPeriodChange(e,
          u, m, h) && b >= r) {
    ...
    for (b = 0; b < this.data.length; b++)if (n = this.data[b])k = this.parseDates && !this.equalSpacing ? Math.round((n.time - this.startTime) * this.stepWidth + this.cellWidth / 2) :
